...
def genrate_response(llm, question : str , docs: list):
    context = '\n\n'.join(doc for doc in docs)

    SystemMessage = SystemMessage(content=""" You are an expert in talent acquisition that helps determine the best candidate among multiple suitable resumes.
    ...
    response = llm.invoke([SystemMessage, user_message])
    return response.content

def genrate_subquestions(llm, question : str):
    SystemMessage = SystemMessage(context=""" You are an expert in talent acquisition. Separate this job description into 3-4 more focused aspects for efficient resume retrieval. 
    ...
    user_message = HumanMessage(content=f""" Generate 3 to 4 sub-queries based on this initial job description:{question}""")

    oneshot_example = HumanMessage(content=f""" 
    ...
    4. Skills and Qualifications:
      ...
      - Strong communication and project management abilities.
                             """)
    response = llm.invoke([SystemMessage, oneshot_example,oneshot_response,user_message])